...
def main():
    ...
    parser.add_argument('--result_path', dest='result_path',
                        default='test_taihe_finetune_96epoch_result22/',
    ...
    center = 'D'
    center_result_path = os.path.join(result_path, 'result', center)
    ...
    for sub in subs:
        print(f"Calculating metrics for subject: {sub[:2]}")

        # 读取GT和合成图像
        gt_file = os.path.join(gt_nii_path, sub[:2] + '.nii')
        syn_file = os.path.join(syn_nii_path, sub[:2] + '.nii')

        if not os.path.exists(gt_file) or not os.path.exists(syn_file):
            print(f"Warning: Files not found for subject {sub[:2]}, skipping...")
            continue
        ...
        mae_result, mae_fore = mae_3d0(ct_gt, ct_syn_aug)
        ...
        print(f"Subject {sub[:2]} - SSIM: {ssim_3d_result:.4f}, PSNR: {psnr_fore_3d_result:.4f}, MAE: {mae_result:.4f}")
        print(f"Subject {sub[:2]} - MAE_Bone: {mae_bone:.4f}, MAE_Tissue: {mae_tissu:.4f}")
